...
This section presents the results acquired from the Fitts' Law target reaching test. The test had five measures which each expresses a parameter of subjects' performance. Subjects were divided into two groups, one test group which received continuous classifications scores during user training, and a control group which received single class classification scores during user training. The plotted mean and standard deviations of each measure in the performance test for session can be seen in /figref{thereIsnoFigRefYet}

\end{multicols}

%throughput
\begin{figure}[H] 
	\subfigure[Throughput measure for the Fitts' Law test between the test and control group.]
...
\begin{figure}[H] 
	\centering
	\subfigure[Path efficiency measure for the Fitts' Law test between the test and control group. There is no significant difference between the groups ($p > 0.05$).]
	{\includegraphics[width=.42\textwidth]{figures/xWesulds/PathEfficiency}} 
	\subfigure[Overshoot measure for the Fitts' Law test between the test and control group. There is no significant difference between the groups ($p > 0.05$).]
...
\begin{figure}[H] 
	\centering
	\subfigure[Stopping distance measure for the Fitts' Law test between the test and control group. There is no significant difference between the groups ($p > 0.05$).]
	{\includegraphics[width=.42\textwidth]{figures/xWesulds/StoppingDistance}} 
	\subfigure[Completion rate measure for the Fitts' Law test between the test and control group. There is no significant difference between the groups ($p > 0.05$).]
...
Comparing the ability to reach different positions within the training showed no significant difference between the three sessions  for both groups ($p > 0.05$).\\ %, with the Tukey-Kramer correction resulting in $p > 0.05$ between all sessions for both the test and control group, meaning there was no development in ability to reach specific positions. 
